...
import { ChatMessage, createOrGetSession, updateMessageMediaData, updateMessageMediaUrl } from "@/lib/chat-storage";
...
import { Blocks, Maximize2, RefreshCw } from "lucide-react";
 ...
 */
export const MessageBubble = memo(function MessageBubble({ msg, onUpdate, charName, userName, characterId, onMusicPlay, onActionSelect, displayContent, defaultTranslationExpanded = false }: MessageBubbleProps) {
    ...
    // Skip function props (onUpdate) — they're inline and always new
...
function MarkdownTextContent({
    ...
    if (!hasHtmlBlocks) {
        ...
        const mdCleaned = linkifyBareUrls(body.trim());
        if (!mdCleaned && !styles) return null;
        return (
    ...
    return (
        <div className="chat-markdown hide-scrollbar break-words" ref={containerRef}>
            {segments.map((seg, i) => {
                ...
                const mdContent = linkifyBareUrls(body.trim());
                return (
                    <div key={`md-${i}`}>
                        {styles && <style dangerouslySetInnerHTML={{ __html: styles }} />}
                        {mdContent && (
