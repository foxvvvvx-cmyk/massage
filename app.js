...
let config={apiKey:'',activePersonaId:'shendu'},personas=[],memories=[],diaries=[],anniversaries=[],balanceCache=null
...
const messagesEl=$('messages'),inputEl=$('input'),sendBtn=$('sendBtn'),voiceBtn=$('voiceBtn'),hintBox=$('hintBox'),hintTag=$('hintTag'),toastEl=$('toast'),drawerEl=$('drawer'),drawerOverlay=$('drawerOverlay'),personaListEl=$('personaList'),personaFormEl=$('personaForm'),personaModalOverlay=$('personaModalOverlay'),confirmModalOverlay=$('confirmModalOverlay')
...
function appendMsgEl(msg){if(msg.type==='system'){const e=document.createElement('div');e.className='msg system';e.textContent=msg.content;messagesEl.appendChild(e);return}
...
const e=document.createElement('div');e.className='msg '+(msg.role==='user'?'user':'ai');e.innerHTML=escHtml(msg.content)+'<div class="time">'+fmtTime(msg.ts)+'</div>';messagesEl.appendChild(e)}
...
async function send(){
  ...
  hintBox.style.display='none';const um={role:'user',content:t,ts:Date.now()};activeHistory().push(um);savePersonas();appendMsgEl(um)
  ...
  try{
    ...
    const res=await fetch(DEEPSEEK_CHAT,{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+config.apiKey},body:JSON.stringify({model:p.useReasoner?'deepseek-reasoner':(p.model||'deepseek-chat'),messages:msgs,temperature:p.temperature??1.3,top_p:p.topP??0.9,max_tokens:4096})})
    if(!res.ok){const et=await res.text();let em;if(res.status===401)em='API Key 无效，请去设置页检查';else if(res.status===402)em='余额不足，请充值';else if(res.status===429)em='请求太频繁，稍等一下';else em=res.status+' '+et.slice(0,60);throw new Error(em)}
    const data=await res.json(),choice=data.choices?.[0]?.message||{},reply=choice.content||'……',reasoning=choice.reasoning_content||null
    const bm={role:'assistant',content:reply,reasoning,ts:Date.now()};activeHistory().push(bm);savePersonas()
    hideTyping();appendMsgEl(bm);messagesEl.scrollTop=messagesEl.scrollHeight;fetchBalance()
...
function renderSettings(){
  ...
  $('settingsContent').innerHTML=`<div class="settings-section"><div class="sec-title">API 设置</div><label>DeepSeek API Key</label><input id="setApiKey" type="password" value="${escHtml(config.apiKey||'')}" placeholder="sk-xxxxxxxx" autocomplete="off"><div class="settings-hint"><a href="https://platform.deepseek.com/api_keys" target="_blank">获取 API Key</a>，新用户有免费额度</div><div class="balance-row"><span class="bl">账户余额</span><span class="bv" id="balanceVal">${balanceCache||'--'}</span></div><div style="text-align:right;margin-top:4px"><span style="font-size:10px;color:var(--text-muted);cursor:pointer;text-decoration:underline" onclick="fetchBalance()">刷新余额</span></div></div><div class="settings-section"><div class="sec-title">当前角色：${p.avatar} ${escHtml(p.name)}</div><p style="font-size:11px;color:var(--text-muted);margin-bottom:6px">编辑角色请在聊天页左上角打开抽屉，点击角色旁的 ✎</p><button class="btn-full" onclick="switchTab('chat');setTimeout(openDrawer,300)">打开角色列表</button></div><div class="settings-section"><div class="sec-title">数据管理</div><div class="btn-row"><button class="btn-primary" onclick="exportAll()" style="flex:1">导出备份</button><button class="btn-outline" onclick="document.getElementById('importFile').click()" style="flex:1">导入备份</button></div><input type="file" id="importFile" accept=".json" style="display:none" onchange="importAll(this)"><button class="btn-full" onclick="clearAllData()">清空所有数据</button></div><button class="btn-full primary" onclick="saveSettingsFromForm()">保存设置</button>`
  fetchBalance()
}

function saveSettingsFromForm(){config.apiKey=($('setApiKey')?.value||'').trim();saveConfig();updateChatHeader();fetchBalance();toast('设置已保存')}
...
function migrateOldData(){
  ...
  if(oc&&!localStorage.getItem(LS_CONFIG)){try{const c=JSON.parse(oc);config.apiKey=c.apiKey||'';saveConfig()}catch(e){}}
  if(oh&&personas.length>0){try{const h=JSON.parse(oh);const s=personas.find(p=>p.id==='shendu');if(s&&(!s.chatHistory||s.chatHistory.length===0)){s.chatHistory=h.filter(m=>m.role==='user'||m.role==='assistant');savePersonas()}}catch(e){}}
...
;(function init(){
  load();migrateOldData();updateChatHeader()
  if(hintBox)hintBox.querySelector('.hint-greeting').textContent=getGreeting()
  renderAllMessages();if(config.apiKey)fetchBalance();registerSW()
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeDrawer();closePersonaModal();closeConfirm()}})
  console.log('🌙 沈度 v5 蓝螺钿已就绪')
