...
let config={apiKey:'',apiProvider:'deepseek',openrouterKey:'',openrouterModel:'anthropic/claude-sonnet-4.6',customBaseUrl:'',customApiKey:'',customModel:'',activePersonaId:'shendu',lockPasscode:'',chatBg:'',userAvatar:'',userName:'',deepThink:false,fontSize:'m',theme:'abyss',autoSync:false,lastSyncTime:0,jealousyLevel:50,jealousyStyle:'撒娇',touchSkin:true,visionEnabled:false,visionBaseUrl:'',visionApiKey:'',visionModel:''},personas=[],memories=[],diaries=[],anniversaries=[],favorites=[],reminders=[],bookmarks=[],rooms=[],sessions=[],moments=[],balanceCache=null
...
function stripThinkingTags(text){
  // Strip <thinking>...</thinking> and [内心]...[/内心] tags from inline content
  return text.replace(/<thinking>[\s\S]*?<\/thinking>/gi,'').replace(/\[内心\][\s\S]*?\[\/内心\]/gi,'').replace(/<response>|<\/response>/gi,'').trim()
}
...
async function describeImages(images){
  if(!config.visionEnabled||!config.visionBaseUrl||!config.visionApiKey||!config.visionModel){
    return null // vision not configured, skip
  }
...
async function send(){
  ...
  // Set generating early to prevent double-send during image recognition
  ...
  // Image description: use vision model to describe images before sending to DeepSeek
  let imageDesc=''
  if(pendingImages.length>0&&config.visionEnabled&&config.visionApiKey&&config.visionBaseUrl&&config.visionModel){
    toast('👁️ 正在识别图片内容…')
    const desc=await describeImages(pendingImages)
    if(desc){imageDesc='\n\n[图片内容：'+desc+']'}
  }
  const finalContent=t+(imageDesc||'')||(pendingImages.length>0?'[图片]':'')
  ...
  try{
    ...
    while(true){const{value,done}=await reader.read();if(done)break;buf+=decoder.decode(value,{stream:true})
      ...
      for(const line of lines){if(!line.startsWith('data: '))continue;const d=line.slice(6);if(d==='[DONE]'){buf='';break}
        try{const j=JSON.parse(d);const delta=j.choices?.[0]?.delta;if(delta?.content){bm.content+=delta.content;const raw=bm.content
          // During streaming, hide thinking/内心 block (robust regex-based detection)
          const thinkTagRe=/<\s*\/?\s*(thinking|Thinking|THINKING)\s*>/i
          const innerTagRe=/\[\s*\/?\s*内心\s*\]/i
    ...
    // Parse <thinking> / [内心] / <response> tags from content
    ...
    if(thinkMatch){
      ...
      if(respMatch){
        ...
        // Fallback: remove the thinking tag and everything in between
  ...
  if(section==='favs'){
    ...
    const bgStyle=config.chatBg?`background-image:url(${escHtml(config.chatBg)});background-size:cover;background-position:center`:''
      ...
      <div class="settings-section"><div class="sec-title">☁️ Supabase 云端记忆</div>
	      ...
	      <div class="settings-section"><div class="sec-title">👁️ 图片识别（视觉模型代理）</div>
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
          <input type="checkbox" id="setVisionEnabled" ${config.visionEnabled?'checked':''} style="width:auto;accent-color:var(--accent)">
          <label style="margin:0;cursor:pointer" onclick="document.getElementById('setVisionEnabled').click()">启用图片识别</label>
        </div>
        <div class="settings-hint">发图片时先用视觉模型转成文字描述，再传给 DeepSeek</div>
        <label style="margin-top:8px">Base URL（OpenAI 兼容）</label>
        <input id="setVisionBaseUrl" value="${escHtml(config.visionBaseUrl||'')}" placeholder="https://dashscope.aliyuncs.com/compatible-mode/v1">
        <div class="settings-hint">通义千问VL、GPT-4V、Gemini等兼容接口</div>
        <label style="margin-top:8px">API Key</label>
        <input id="setVisionApiKey" type="password" value="${escHtml(config.visionApiKey||'')}" placeholder="sk-xxxxxxxx" autocomplete="off">
        <label style="margin-top:8px">模型名</label>
        <input id="setVisionModel" value="${escHtml(config.visionModel||'')}" placeholder="qwen-vl-plus">
        <div class="settings-hint">如：qwen-vl-plus、gpt-4o、gemini-2.0-flash-exp</div>
      </div>
...
function saveSettingsFromForm(){
  ...
  config.visionBaseUrl=($('setVisionBaseUrl')?.value||'').trim()
  config.visionApiKey=($('setVisionApiKey')?.value||'').trim()
  config.visionModel=($('setVisionModel')?.value||'').trim()
