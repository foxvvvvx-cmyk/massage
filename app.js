...
const LS_MEMORIES='sd_v5_memories';const LS_DIARIES='sd_v5_diaries';const LS_ANNIVERSARIES='sd_v5_anniversaries';const LS_FAVORITES='sd_v5_favorites'
const COMMON_EMOJIS=['🌙','🖤','🤍','🍃','🌸','💫','⭐','🌺','🦋','🐱','🎀','💎','🔥','🌈','🎵','☕']
...
let config={apiKey:'',activePersonaId:'shendu',lockPasscode:'',wallpaper:''},personas=[],memories=[],diaries=[],anniversaries=[],favorites=[],balanceCache=null
...
let pendingImages=[],searchResults=[],searchIdx=-1
...
function afterUnlock(){updateChatHeader();if(hintBox)hintBox.querySelector('.hint-greeting').textContent=getGreeting();renderAllMessages();if(config.apiKey)fetchBalance();applyWallpaper();updateStatusBar()}
...
function buildMsgHTML(msg){
  ...
  return `<div class="${cls}" data-ts="${msg.ts}">${imgHTML}${escHtml(msg.content)}<div class="time">${fmtTime(msg.ts)}</div>${favHTML}${reactionsHTML}</div>`
}
...
function showCtxMenu(msg,e){
  ...
  const favBtn=ctxMenu.querySelector('button:nth-child(2)')
...
function extractKeywords(text){
  // simple Chinese tokenizer: split by non-CJK, filter stop words, extract 1-4 char segments
  ...
  for(const seg of segments){
    ...
    // Chinese: slide window to extract meaningful substrings
    if(/[一-鿿]/.test(seg)){
      if(!CN_STOP_WORDS.has(seg)&&seg.length>=1) keywords.push(seg)
      // extract 2-char combos for longer strings
      if(seg.length>=4){
...
function scoreMemory(memory,keywords,now){
  ...
  // bonus: recently used memory gets +1
...
function getRelevantMemories(userMessage){
  ...
  const scored=memories.map(m=>({mem:m,score:scoreMemory(m,keywords,now)})).filter(s=>s.score>=3)
...
function buildMemoryInject(matched){
  ...
  const items=matched.map(m=>`- ${m.content} | 来源：${m.source==='auto'?'自动记录':'手动记录'}`).join('\n')
...
async function extractMemoriesFromChat(silent){
  if(isExtracting||!config.apiKey)return
  isExtracting=true
  try{
    // collect last 20 messages (user + assistant) from active persona
    const h=activeHistory()
    const recent=h.filter(m=>m.role==='user'||m.role==='assistant').slice(-20)
    const userMsgs=recent.filter(m=>m.role==='user')
    if(userMsgs.length<3)return // not enough conversation
    ...
    for(const line of lines){
      ...
      const fact=line.slice(idx+1).trim()
      if(!fact||fact.length<2)continue
      // dedup: check if similar fact already exists
      const exists=memories.some(m=>{
        const overlap=m.content.replace(/[^一-鿿]/g,'')
        const nOverlap=fact.replace(/[^一-鿿]/g,'')
      ...
      if(!exists){
        memories.unshift({id:Date.now()+added,content:fact,category:cat||'默认',tags:extractKeywords(fact).slice(0,5),usageCount:0,lastUsed:null,source:'auto',createdAt:Date.now()})
        added++
      }
    }
...
function attachImage(inp){
  ...
  if(!f.type.startsWith('image/')){toast('请选择图片文件');inp.value='';return}
  ...
  reader.onload=function(e){
    ...
    img.onload=function(){
      ...
      pendingImages.push({dataUrl,mimeType:'image/jpeg'})
      renderImagePreview()
    }
...
// ===== DIARY TRIGGER FROM CHAT =====
function askAiDiary(){
  switchTab('chat')
  inputEl.value='帮我写一篇日记吧，记录一下最近的心情'
...
function toggleFavorite(ts){
  ...
  if(idx>=0){favorites.splice(idx,1);saveFavorites();renderAllMessages();toast('已取消收藏')}
    ...
    const h=activeHistory();const m=h.find(m=>m.ts===ts)
    if(!m)return
    favorites.unshift({ts:m.ts,content:m.content,role:m.role,personaId:config.activePersonaId,savedAt:Date.now()})
    saveFavorites();renderAllMessages();toast('⭐ 已收藏')
  }
}
function goToFavorite(ts){
  switchTab('chat');const h=activeHistory();const m=h.find(m=>m.ts===ts)
  if(!m){toast('消息已不存在');return}
  setTimeout(()=>scrollToMessage(ts),300)
}

// ===== SEARCH =====
function toggleSearch(){
  const sw=$('searchWrap'),si=$('searchInput')
  if(!sw||!si)return
...
function searchMessages(){
  ...
  const si=$('searchInfo');if(!si)return
  if(!q){searchResults=[];searchIdx=-1;si.textContent='';renderAllMessages();return}
  const h=activeHistory()
  searchResults=h.map((m,i)=>({m,i,text:(m.content||'').toLowerCase()})).filter(r=>r.text.includes(q));searchIdx=-1
  si.textContent=searchResults.length+' 条结果'
  // highlight matching messages in DOM
  renderAllMessages()
  if(searchResults.length>0){
    // scroll to first result
    scrollToMessage(searchResults[0].m.ts)
    // highlight all results
    const qesc=escHtml(q)
    document.querySelectorAll('.msg').forEach(el=>{
      const ts=parseInt(el.dataset.ts)
      if(searchResults.some(r=>r.m.ts===ts)){
        el.classList.add('search-result')
        el.innerHTML=el.innerHTML.replace(new RegExp('('+qesc.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+')','gi'),'<span class="search-highlight">$1</span>')
      }
    })
  }
}
function scrollToMessage(ts){
  ...
  if(el){el.scrollIntoView({behavior:'smooth',block:'center'});el.style.animation='none';el.offsetHeight;el.style.animation=''}
}
...
async function send(){
  ...
  try{
    ...
    // build system prompt with memory rules
    ...
    activeHistory().slice(-24).forEach(m=>{
      if(m.images&&m.images.length&&m.role==='user'){
        // multimodal message
        const parts=m.images.map(img=>({type:'image_url',image_url:{url:img.dataUrl}}))
        if(m.content)parts.push({type:'text',text:m.content})
        msgs.push({role:m.role,content:parts})
      }else{msgs.push({role:m.role,content:m.content})}
    })
    // slightly lower temperature when memories are injected for accuracy
    const temp=matched.length>0?Math.max(0.3,(p.temperature??1.3)-0.15):(p.temperature??1.3)
    const res=await fetch(DEEPSEEK_CHAT,{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+config.apiKey},body:JSON.stringify({model:p.useReasoner?'deepseek-reasoner':(p.model||'deepseek-chat'),messages:msgs,temperature:temp,top_p:p.topP??0.9,max_tokens:4096,stream:true})})
    ...
    // detect diary markers: 【日记】...【/日记】
    const diaryMatch=/【日记】([\s\S]*?)【\/日记】/.exec(bm.content)
    if(diaryMatch){
      const diaryContent=diaryMatch[1].trim()
      if(diaryContent&&diaryContent.length>=15){
        const dts=Date.now()
        diaries.unshift({id:dts,content:diaryContent,ts:dts,mood:'🤖',timeLabel:timeOfDay(dts),source:'ai'})
        saveDiaries()
        const cleanContent=bm.content.replace(/【日记】[\s\S]*?【\/日记】/,'').trim()
        bm.content=cleanContent||bm.content
        savePersonas() // save cleaned message
        el.innerHTML=escHtml(bm.content)+'<div class="diary-saved-hint">📔 已存入日记</div><div class="time">'+fmtTime(bm.ts)+'</div>'
        setTimeout(()=>toast('📔 日记已自动保存'),400)
      }else{el.innerHTML=escHtml(bm.content)+'<div class="time">'+fmtTime(bm.ts)+'</div>'}
    }else{el.innerHTML=escHtml(bm.content)+'<div class="time">'+fmtTime(bm.ts)+'</div>'}
  ...
  // auto memory extraction after every 8 user messages
  autoExtractCount++
  if(autoExtractCount>=8){autoExtractCount=0;extractMemoriesFromChat(true)}
}
...
function estimateContextTokens(){
  ...
  return Math.ceil(chars/2) // rough: ~2 chars per token for mixed CN/EN
}
function updateStatusBar(){
  const bv=$('sbBalanceVal');if(bv)bv.textContent=balanceCache||'--'
  const cv=$('sbContextVal');if(cv){
    const tokens=estimateContextTokens(),max=65536,pct=Math.min(100,Math.round(tokens/max*100))
...
function exportAll(){const d={version:'v5',exportedAt:new Date().toISOString(),config:{activePersonaId:config.activePersonaId},personas:personas.map(p=>({...p,chatHistory:p.chatHistory||[]})),memories,diaries,anniversaries,favorites};const b=new Blob([JSON.stringify(d,null,2)],{type:'application/json'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download='沈度备份_'+dayKey(Date.now())+'.json';a.click();URL.revokeObjectURL(u);toast('已导出')}
function importAll(inp){const f=inp.files[0];if(!f)return;const r=new FileReader();r.onload=e=>{try{const d=JSON.parse(e.target.result);if(!d.version)throw new Error('格式不对');showConfirm('确认导入','将导入：\n· '+(d.personas?.length||0)+' 个角色\n· '+(d.memories?.length||0)+' 条记忆\n· '+(d.diaries?.length||0)+' 条日记\n· '+(d.favorites?.length||0)+' 条收藏\n当前数据会被覆盖，确定？',()=>{if(d.personas)personas=d.personas;if(d.memories)memories=d.memories;if(d.diaries)diaries=d.diaries;if(d.anniversaries)anniversaries=d.anniversaries;if(d.favorites)favorites=d.favorites;if(d.config?.activePersonaId)config.activePersonaId=d.config.activePersonaId;savePersonas();saveMemories();saveDiaries();saveAnniversaries();saveFavorites();saveConfig();updateChatHeader();renderAllMessages();renderSettings();toast('已导入')})}catch(err){toast('文件格式错误')}};r.readAsText(f);inp.value=''}
function clearAllData(){showConfirm('确认清空','将删除所有角色、聊天记录、记忆、日记，不可恢复。确定？',()=>{personas=JSON.parse(JSON.stringify(DEFAULT_PERSONAS));memories=[];diaries=[];anniversaries=[];favorites=[];config.activePersonaId='shendu';savePersonas();saveMemories();saveDiaries();saveAnniversaries();saveFavorites();saveConfig();updateChatHeader();renderAllMessages();renderSettings();toast('已清空')})}
...
function renderFavoritesHTML(){
  ...
  const items=favorites.slice(0,5).map(f=>{
    const preview=f.content?f.content.slice(0,60)+(f.content.length>60?'…':''):'[图片]'
    return `<div class="fav-item" onclick="goToFavorite(${f.ts})"><span class="fav-role ${f.role==='user'?'user':'ai'}">${f.role==='user'?'我':'AI'}</span><span class="fav-preview">${escHtml(preview)}</span><div class="fav-meta">${fmtDate(f.savedAt)}</div><button class="fav-unstar" onclick="event.stopPropagation();toggleFavorite(${f.ts});renderDashboard()">✕</button></div>`
  }).join('')
...
function migrateOldData(){
  ...
  // patch old memories missing new fields
  let memPatched=false
  memories.forEach(m=>{if(!m.source){m.source='manual';memPatched=true}if(!m.tags){m.tags=[];memPatched=true}if(m.usageCount===undefined){m.usageCount=0;memPatched=true}if(m.lastUsed===undefined){m.lastUsed=null;memPatched=true}})
