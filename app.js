...
function renderMe(){
  ...
  if(section==='favs'){
    ...
    c.innerHTML=`
      ...
      <div style="margin-top:6px"><span style="font-size:10px;color:var(--text-muted);cursor:pointer;text-decoration:underline" onclick="fetchBalance();setTimeout(()=>renderMe(),500)">余额：${balanceCache||'--'}（点击刷新）</span></div></div>
      ...
      ${all.length>0?renderMoodChart():'<div class="mem-empty">还没有聊天记录</div>'}`
  }else{
